sap.ui.define(["sap/ui/base/Object"], function(object) {
	...
	var fnGetstockNos = function(stockType) {
		...
		var ifUrl = this.target + "/list=";
		ifUrl += stockType;
		for (var i = this.startNo; i < this.endNo; i++) {
			var targetUrl = ifUrl + String.padd("R", 6, i.toString(), "0");
			$.ajax(targetUrl, {
	...
	var fnGetHistoryData = function(url) {
		window["context"] = this;
		$.ajax(url, {
			success: fnGetHistoryDataCallBack.bind(this),
			async: false
		});
	};

	var fnGetHistoryDataCallBack = function(data, txtStatus, jsXHR) {
	...
	return object.extend("ZHELI_UI5_CONTROLS.model.stockHelp", {
		...
		getHistoryData: fnGetHistoryData
